...
class SHkeys(object):
    ...
    def Shaveoff_k_nterms_for_m_gt(self,k,mmin):
        """ remove keys if n - m is odd """
        nmax = self.n.max()
        keycheck = lambda key: (key[0] <= (nmax - 2) ) or (key[1] <= mmin)

        self.keys = tuple([key for key in self.keys if keycheck(key)])
        self.make_arrays()
        return self
...
def nterms_analytic_Ephi_zero(NT = 0, MT = 0, Nmin = 1):
    ...
    return len(SHkeys(NT , MT ).setNmin(Nmin).MleN().Mge(0).Shaveoff_k_nterms_for_m_gt(2,0)) + \
           len(SHkeys(NT , MT ).setNmin(Nmin).MleN().Mge(1).Shaveoff_k_nterms_for_m_gt(2,0))
...
def get_legendre_arrays__Amatrix(nmax, mmax, theta, keys,
    ...
    # keycheck = lambda key: (key[0] <= (nmax - 2) ) or (key[1] == 0)
    # # Pmat  = np.hstack(tuple(P[key] for key in keys if key[0] <= (nmax-2)))
    # # dPmat = np.hstack(tuple(dP[key] for key in keys if key[0] <= (nmax-2)))
    # Pmat  = np.hstack(tuple(P[key] for key in keys if keycheck(key)))
    # dPmat = np.hstack(tuple(dP[key] for key in keys if keycheck(key)))
    # return Pmat, dPmat, A
...
def getG_torapex_dask_analyticEphi_zero(NT, MT, alat, phi, 
    ...
    # generate spherical harmonic keys    
    keys = {} # dictionary of spherical harmonic keys
    keys['cos_T'] = SHkeys(NT, MT).setNmin(1).MleN().Mge(0).Shaveoff_k_nterms_for_m_gt(2,0)
    keys['sin_T'] = SHkeys(NT, MT).setNmin(1).MleN().Mge(1).Shaveoff_k_nterms_for_m_gt(2,0)
