...
#20-22h, early
earlyinds = (dfhemi["onset_mlt"] > 21.5) & (dfhemi["onset_mlt"] < 23.5)

Byposearlyinds = Byposinds & earlyinds
Bynegearlyinds = Byneginds & earlyinds

#1-3h, late
lateinds = (dfhemi['onset_mlt'] > 0.5) & (dfhemi['onset_mlt'] < 2.5)
...
names = ['By > 0, early','By < 0, late','By > 0, late','By < 0, late']
...
#%% Binne data
    #Regner ut for kvar enkelt:
yhatd1meanhemi_Byposearly  = dfhemi[Byposearlyinds].groupby("bin_number")["yhat_d1"].mean()
yhatd2meanhemi_Byposearly  = dfhemi[Byposearlyinds].groupby("bin_number")["yhat_d2"].mean()
yhatbiashemi_Byposearly  = np.sqrt(yhatd1meanhemi_Byposearly**2 + yhatd2meanhemi_Byposearly**2)

yhatd1meanhemi_Bynegearly = dfhemi[Bynegearlyinds].groupby("bin_number")["yhat_d1"].mean()
yhatd2meanhemi_Bynegearly = dfhemi[Bynegearlyinds].groupby("bin_number")["yhat_d2"].mean()
yhatbiashemi_Bynegearly = np.sqrt(yhatd1meanhemi_Bynegearly**2 + yhatd2meanhemi_Bynegearly**2)

yhatd1meanhemi_Byposlate = dfhemi[Byposlateinds].groupby("bin_number")["yhat_d1"].mean()
yhatd2meanhemi_Byposlate = dfhemi[Byposlateinds].groupby("bin_number")["yhat_d2"].mean()
yhatbiashemi_Byposlate = np.sqrt(yhatd1meanhemi_Byposlate**2 + yhatd2meanhemi_Byposlate**2)

yhatd1meanhemi_Byneglate = dfhemi[Byneglateinds].groupby("bin_number")["yhat_d1"].mean()
yhatd2meanhemi_Byneglate = dfhemi[Byneglateinds].groupby("bin_number")["yhat_d2"].mean()
yhatbiashemi_Byneglate = np.sqrt(yhatd1meanhemi_Byneglate**2 + yhatd2meanhemi_Byneglate**2)


# Lage kunstig data
countdata = [yhatbiashemi_Byposearly,
             yhatbiashemi_Bynegearly,
             yhatbiashemi_Byposlate,
             yhatbiashemi_Byneglate]

# bli kvitt "-1"-bin
countdata = [count[1:] for count in countdata]

names = ['By > 0, early','By < 0, late','By > 0, late','By < 0, late']
cbnames = ['Bias','Bias', "Bias", "Bias"]
...
#%% Snittvinkel
dfhemi["yhd1abs"] = dfhemi['yhat_d1'].abs()
dfhemi["yhd2abs"] = dfhemi['yhat_d2'].abs()


yhatd1absmeanhemi_Byposearly = dfhemi[Byposearlyinds].groupby("bin_number")["yhd1abs"].mean()
yhatd2absmeanhemi_Byposearly = dfhemi[Byposearlyinds].groupby("bin_number")["yhd2abs"].mean()
...
yhatbiasvhemi_Byposearly = np.rad2deg(np.arctan2(yhatd2absmeanhemi_Byposearly,yhatd1absmeanhemi_Byposearly))
#yhatbiasS = np.sqrt(yhatd1meanS**2 + yhatd2meanS**2)

yhatd1absmeanhemi_Bynegearly = dfhemi[Bynegearlyinds].groupby("bin_number")["yhd1abs"].mean()
yhatd2absmeanhemi_Bynegearly = dfhemi[Bynegearlyinds].groupby("bin_number")["yhd2abs"].mean()
yhatbiasvhemi_Bynegearly = np.rad2deg(np.arctan2(yhatd2absmeanhemi_Bynegearly, yhatd1absmeanhemi_Bynegearly))

yhatd1absmeanhemi_Byposlate = dfhemi[Byposlateinds].groupby("bin_number")["yhd1abs"].mean()
yhatd2absmeanhemi_Byposlate = dfhemi[Byposlateinds].groupby("bin_number")["yhd2abs"].mean()
yhatbiasvhemi_Byposlate= np.rad2deg(np.arctan2(yhatd2absmeanhemi_Byposlate,yhatd1absmeanhemi_Byposlate))


yhatd1absmeanhemi_Byneglate = dfhemi[Byneglateinds].groupby("bin_number")["yhd1abs"].mean()
yhatd2absmeanhemi_Byneglate = dfhemi[Byneglateinds].groupby("bin_number")["yhd2abs"].mean()
yhatbiasvhemi_Byneglate = np.rad2deg(np.arctan2(yhatd2absmeanhemi_Byneglate,yhatd1absmeanhemi_Byneglate))
...
# Lage kunstig data
countdata = [yhatbiasvhemi_Byposearly,
             yhatbiasvhemi_Bynegearly,
             yhatbiasvhemi_Byposlate,
             yhatbiasvhemi_Byneglate ]

# bli kvitt "-1"-bin
countdata = [count[1:] for count in countdata]

names = ['By > 0, early','By < 0, late','By > 0, late','By < 0, late']
cbnames = ['Snittvinkel','Snittvinkel', "Snittvinkel", "Snittvinkel"]
