...
from utils import nterms_analytic_Ephi_zero, SHkeys, getG_torapex_dask_analyticEphi_zero, make_model_coeff_txt_file_analyticEphi_zero
...
MODELSUFF = '_analyticEphizero_at_47deg'
MODELSUFF = '_analyticEphizero_at_'+",".join([f"{this:.0f}" for this in zero_lats])+'deg'
...
def itersolve(filename):
    ...
    while True:
        ...
        try:
            ...
            n_cos_T = SHkeys(NT, MT).setNmin(1).MleN().Mge(0).Shaveoff_k_nterms_for_m_gt(2,0).n
            n_sin_T = SHkeys(NT, MT).setNmin(1).MleN().Mge(1).Shaveoff_k_nterms_for_m_gt(2,0).n
...
G0 = getG_torapex_dask_analyticEphi_zero(NT, MT, 
